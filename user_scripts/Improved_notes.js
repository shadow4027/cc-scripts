...
(function() {
    ...
    const NOTE_TEMPLATES = [
        {"name": "CSR", "value": "CSR_Normal", "template": ["Name: ", "Verified: ", "Phone: ", "Issue: ", "Resolution: "]},
    ...
    unsafeWindow.NOTE_TEMPLATES = NOTE_TEMPLATES;
    // functions
    ...
    let inject_template = async function () {
        ...
        for(let template_index in NOTE_TEMPLATES) {
            ...
            let five9_phonenumber = GM.getItem(Five9PhoneNumber, null);
            if (selected_value === template.value) {
                //create the string from the list inside the date structure.
                if (five9_phonenumber === null) { 
                    for (let template_list_index in template.template) {
                        template_string += template.template[template_list_index] + '\n';
                    }   
                }
                else{
                    for (let template_list_index in template.template) {
                        template_string_part = template.template[template_list_index] + '\n';
                        template_string += template_string.replace("%phonenumber%", Five9PhoneNumber);
                    }   
                }
                break;
            }
